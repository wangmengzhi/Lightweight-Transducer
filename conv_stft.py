...
class STFT(torch.nn.Module):
    def __init__(self, win_len=1024, win_hop=512, fft_len=1024,
                 enframe_mode='continue', win_type='hamm',
    ...
    def __init_kernel__(self):
        ...
        ** overlap-add kernel, just like enframe_kernel, but transposed.
        ...
        fft_kernel = torch.fft.rfft(torch.eye(self.fft_len), dim=1)
        if self.mode == 'break':
            enframed_kernel = torch.eye(self.win_len)[:, None, :]
            fft_kernel = fft_kernel[:self.win_len]
        fft_kernel = torch.cat((fft_kernel.real, fft_kernel.imag), dim=1)
    ...
    def transform(self, inputs, return_type='magphase'):
        ...
        if self.pad_center:
            inputs = tf.pad(inputs, (self.pad_amount, self.pad_amount), mode='reflect')
        ...
        outputs = tf.conv1d(enframe_inputs, self.fft_k.unsqueeze(-1))
